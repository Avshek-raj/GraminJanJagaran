...
import { generateProgramDescription } from '../services/geminiService';
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
  ...
  const [activeTab, setActiveTab] = useState<'programs' | 'members' | 'donations'>('programs');
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-12">
        <div>
          ...
          <p className="text-slate-500 mt-1">Manage your NGO programs, team, and donations.</p>
        </div>
        
        <div className="mt-6 md:mt-0 flex p-1.5 bg-slate-100 rounded-2xl border border-slate-200">
          {(['programs', 'members', 'donations'] as const).map(tab => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`px-8 py-2.5 rounded-xl font-bold text-xs uppercase tracking-widest transition-all capitalize ${
                activeTab === tab ? 'bg-white text-emerald-600 shadow-md' : 'text-slate-500 hover:text-slate-900'
              }`}
            >
              {tab}
            </button>
        ...
        <div className="animate-fade-in">
          <div className="flex justify-between items-center mb-10">
            <div className="flex items-center space-x-3">
              <h2 className="text-2xl font-black text-slate-900">Manage Programs</h2>
              <span className="px-3 py-1 bg-slate-100 rounded-full text-slate-500 text-xs font-bold">{programs.length} total</span>
            </div>
            <button 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {programs.map(prog => (
              <div key={prog.id} className={`bg-white p-8 rounded-3xl shadow-sm border flex flex-col transition-all ${prog.status === 'completed' ? 'border-emerald-100' : 'border-slate-100'}`}>
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <div className="flex items-center space-x-2 mb-1">
                      <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full ${prog.status === 'completed' ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-500'}`}>
                        {prog.status}
                      </span>
                ...
                <div className="space-y-4 pt-6 border-t border-slate-50">
                  ...
                  >
                    {prog.status === 'active' ? 'Mark Completed' : 'Mark Active'}
                  </button>
                </div>
              </div>
            ))}
                ...
                <form onSubmit={submitProgram} className="space-y-6">
                  <div className="grid grid-cols-2 gap-6">
                    <div className="col-span-2 md:col-span-1">
                      ...
                      <input required value={progTitle} onChange={e => setProgTitle(e.target.value)} className="w-full px-5 py-3 rounded-2xl border-2 border-slate-50 focus:border-emerald-500 focus:bg-white bg-slate-50 outline-none transition-all" />
                    </div>
                    <div className="col-span-2 md:col-span-1">
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      <label className="text-xs font-black uppercase tracking-widest text-slate-400">Basic Description</label>
                      <button 
                        type="button" 
                      ...
                      >
                        {aiGenerating ? 'Writing...' : <><i className="fas fa-magic mr-1.5"></i> AI Generate</>}
                      </button>
                    </div>
                  ...
                  {/* Impact Fields (Rich for Success Stories) */}
                  <div className="p-6 bg-emerald-50/50 rounded-2xl border border-emerald-100 space-y-6">
                    <h4 className="text-sm font-black text-emerald-700 flex items-center">
                      <i className="fas fa-star mr-2"></i> Success Story Details
                    </h4>
                    <div>
                      <label className="block text-xs font-black uppercase tracking-widest text-slate-400 mb-2">Detailed Impact Statement (Optional)</label>
                      <textarea value={progImpact} onChange={e => setProgImpact(e.target.value)} className="w-full h-32 px-5 py-4 rounded-2xl border-2 border-white focus:border-emerald-500 bg-white/80 outline-none transition-all resize-none leading-relaxed" placeholder="Tell the full story of what was done..."></textarea>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        <label className="block text-xs font-black uppercase tracking-widest text-slate-400 mb-2">Collaborators (Comma separated)</label>
                        <input value={progCollabs} onChange={e => setProgCollabs(e.target.value)} className="w-full px-5 py-3 rounded-2xl border-2 border-white focus:border-emerald-500 bg-white/80 outline-none transition-all" placeholder="Red Cross, local NGO, etc." />
                      </div>
                      <div>
                        <label className="block text-xs font-black uppercase tracking-widest text-slate-400 mb-2">Gallery URLs (Comma separated)</label>
                        <input value={progGallery} onChange={e => setProgGallery(e.target.value)} className="w-full px-5 py-3 rounded-2xl border-2 border-white focus:border-emerald-500 bg-white/80 outline-none transition-all" placeholder="http://image1.jpg, http://image2.jpg" />
                      </div>
                    </div>
                  </div>

                  <div>
                    <label className="block text-xs font-black uppercase tracking-widest text-slate-400 mb-2">Goal Amount ($)</label>
